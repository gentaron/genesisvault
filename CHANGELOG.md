...
- **VE-010 Tessa Brandt (Scout) — トレンド・レーダー（Phase ν）。** 記事を書く前に、
  ...
  - API キー不要のソースだけを使う。**fail-soft** — 取れなければ `degraded` として
    記録し、記事は通常どおり書く。直近3日以内のスナップショット
    （`data/trend-radar.json`）があれば代用する
  - 相場は予測しない。5営業日の変化率と20日移動平均との位置関係だけを言葉にする
  - ブリーフには必ず使い方の制約が付く（1〜2文まで／投資助言を書かない／
    レーダーの数字をミナ自身の資産額として書かない）
  - 設定は `config/pipeline.json` の `trends`。`bun run verify` からは呼ばない（INV-004）
- **継続性ゲート。** 逆行を「ブリーフでの指示」から「決定論的なチェック」に格上げした。
  検出したら書き手に指摘つきで1度だけ書き直させ、校正後に再確認し、
  テンプレートのフォールバックにも同じ判定を通す（INV-019）。
  回想（「始めた頃は貯金100万円だった」）は矛盾ではないので通す。
- **逆行禁止の対象を金額から全指標へ拡張。** 習慣の継続日数（瞑想・散歩・
  ジャーナリング等）、継続年数（積立投資・ブログ等）、読了冊数を同じ枠組みで扱う。
  読了冊数のように年でリセットされる指標は台帳のキーに年を含める。
- **テーマ「AI・テクノロジー」を追加**（全10テーマ）。`ALL_THEMES` と
  `THEME_KEYWORDS` の一致をテストで固定した。

### Fixed

- **main が進んでいると、その日の記事ごと push が捨てられていた（LM-016）。**
  日次投稿は `git commit && git push` で main へ直接押しており、記事を生成している
  数分の間に PR が 1 本でもマージされていれば non-fast-forward で弾かれた。
  ランナーは使い捨てで再試行も無いので、失われるのは push ではなく**書き上がった
  記事そのもの**だった。`scripts/push-with-rebase.sh` を通し、弾かれたら
  fetch → rebase → 押し直しを指数バックオフで繰り返すようにした。
  - 追記専用ログは `merge=union` が結合する（LM-015）。残る JSON の衝突には
    ファイルごとの方針を持たせた — 導出物（`continuity-ledger` /
    `trend-radar`）は今回の版を採り、追記台帳（`video-briefs`）は
    `mergeLedgers()` で両側を union する
  - **方針の無いファイルが衝突したら解決せずに落とす。** 推測で片側を捨てるより
    赤いジョブのほうが安い
- **継続性台帳が数週間前で凍っていた（LM-013）。** ファイルが在れば再走査しない実装と、
  日次ジョブが台帳をコミットしていなかったことが噛み合い、毎晩更新して毎晩捨てる
  循環になっていた。台帳は毎回 `src/content/posts` から作り直し、workflow の
  `git add` に `data/continuity-ledger.json` と `data/trend-radar.json` を足した。
- **台帳が逆行記事を追認していた（LM-014）。** 「貯金300万達成」の13日後に
  「貯金200万達成」が出来事として並んでいた。到達点を下回る「達成」と、
  出来事への言及にすぎない回想文を記録しないようにした。
- **相場・ニュース由来の数値が個人の事実に混入しうる問題。** 「日経平均が4万2000円」を
  貯金額として台帳に入れると到達不能な下限が居座り、以後すべての原稿が落ちる。
  抽出時に相場語を除外するようにした。
- **記録時にタイトルを走査していなかった問題。** 「貯金300万達成！」のような確定事実は
  本文ではなくタイトルにだけ現れることがある（台帳の再構築側とは扱いが揃っていなかった）。
